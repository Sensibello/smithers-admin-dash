<template>
  <admin-layout
    :collapsed="layout.menuMinimized"
    :compacted="layout.menuCompacted"
    :layout="layout"
    :urlPrefix="urlPrefix"
  />
</template>
...
export default {
    ...
    layout() {
      ...
      if (metaPage.layout) {
        layout = Object.assign(
          layout,
          this.layoutList.layouts[metaPage.layout]
        );
      }
  ...
  head() {
    const metaPage = this.$router.currentRoute.meta;
    const currentSkin =
      this.$store.state.theme.skin !== "default"
        ? this.$store.state.theme.skin
        : this.layout.skin;
    //Enable Header background bg on scroll
    const headerBgOnScroll = this.layout.headerBgOnScroll
      ? " ig-header-bg-on-scroll"
      : "";
